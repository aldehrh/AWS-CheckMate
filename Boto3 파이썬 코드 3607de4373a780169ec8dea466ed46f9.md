...
1. 보안 그룹의 인바운드 규칙 과다 허용
Boto 확인 방법: describe_security_groups()를 호출하여 IpPermissions 리스트 내 변수 IpRanges Cidrlp 키값의 value 값이 0.0.0.0/0을 포함 혹은 단일 설정되어있는지 확인
...
2. 인스턴스 메타데이터 서비스 v2(IMDSv2) 미강제
Boto 확인 방법 describe_instances()의 결과 중 MetadataOptions 필드의 HttpTokens 상태값 확인(True/False)
...
3. EBS 볼륨의 보관 시 암호화(Encryption at rest) 누락
Boto 확인 방법: describe_volumes()로 해당 인스턴스에 연결된 볼륨 정보를 조회한 뒤 Volumes의 필드 Encrpyted 값(T/F) 확인
...
4. IAM 역할 미할당 및 자격 증명 노출
Boto 확인 방법: describe_instances() 결과에서 각 인스턴스 항목에 IamInstanceProfile 존재 하지 않다면 연결된 IAM 역할이 없음
...
5. User 데이터 내 민감 정보 포함
Boto 확인 방법: describe_instance_attribute(InstanceId=’해당인스턴스ID’, Attribute=’userData’)로 UserData 값이 존재한다면 Base64 데이터를 디코딩하여 읽을 수 있는 문자열로 변환하여 확인
